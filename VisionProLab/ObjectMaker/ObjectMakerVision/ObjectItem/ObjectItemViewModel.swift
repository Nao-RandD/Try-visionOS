...
class ObjectItemViewModel: ObservableObject {

    @Published var item: ObjectItem?
    @Published var usdzFileURL: URL?
    @Published var entity: ModelEntity?

    var onItemDeleted: (() -> Void)? = nil

    func listenToItem(_ item: ObjectItem) {
        self.item = item
        Firestore.firestore().collection("items")
            ...
            .addSnapshotListener { [weak self] snapshot, error in
                ...
                self.item = try? snapshot.data(as: ObjectItem.self)
